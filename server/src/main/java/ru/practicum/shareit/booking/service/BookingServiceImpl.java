...
import ru.practicum.shareit.common.Dates;
...
public class BookingServiceImpl implements BookingService {
    ...
    public List<OutputBookingDto> getUsersBooking(Integer userId, String state, Integer from, Integer size) {
        ...
        LocalDateTime now = Dates.getCurrentDateTime();
        ...
        switch (state) {
            ...
            case "REJECTED":
                ...
                break;
            case "CURRENT":
                bookList = bookingRepositary.findBookingByBookerAndStartBeforeAndEndAfter(user, now, now, pageable);
                break;
            case "PAST":
                bookList = bookingRepositary.findBookingByBookerAndEndBefore(user, now, pageable);
                break;
            case "FUTURE":
                bookList = bookingRepositary.findBookingByBookerAndStartAfter(user, now, pageable);
                break;
            default:
    ...
    public List<OutputBookingDto> getBookingsForOwner(Integer userId, String state, Integer from, Integer size) {
        ...
        switch (state) {
            ...
            case "REJECTED":
                ...
                break;
            case "CURRENT":
                bookList = bookingRepositary.getCurrentBookingForOwner(userId, now, now, pageable);
                break;
            case "PAST":
                bookList = bookingRepositary.getPastBookingForOwner(userId, now, pageable);
                break;
            case "FUTURE":
                bookList = bookingRepositary.getFutureBookingForOwner(userId, now, pageable);
                break;
            default:
                String msg = "Unknown state: UNSUPPORTED_STATUS";
                log.info(msg);
    ...
    private void checkBooking(Booking booking) {
        ...
        } else if (booking.getStart().isBefore(Dates.getCurrentDateTime())) {
